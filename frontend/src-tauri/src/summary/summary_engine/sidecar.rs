...
impl SidecarManager {
    ...
    fn resolve_helper_binary() -> Result<PathBuf> {
        ...
        if let Ok(env_path) = std::env::var("FRIDAY_LLAMA_HELPER") {
            if !env_path.is_empty() {
                let path = PathBuf::from(env_path);
                if path.exists() {
                    log::info!("Using llama-helper from FRIDAY_LLAMA_HELPER: {}", path.display());
                    return Ok(path);
                }
            }
        }
        ...
        Err(anyhow!(
            "llama-helper binary not found. Build with 'cd llama-helper && cargo build --release' or set FRIDAY_LLAMA_HELPER env var."
        ))
    }
