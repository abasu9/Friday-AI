...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.friday\meeting_minutes.db",
        "$env:USERPROFILE\Documents\friday\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/friday-backend/*/backend/meeting_minutes.db"
    }
