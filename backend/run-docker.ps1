...
$AppProjectName = "friday-backend"
$AppContainerName = "friday-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
            ...
            "customize" {
                ...
                $dbSelection = switch ($dbChoice) {
                    "2" { 
                        ...
                        Write-Host "Example: C:\Users\YourName\AppData\Local\Friday\meeting_minutes.db"
                        Write-Host ""
    ...
    $composeArgs += @("whisper-server", "friday-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server friday-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server friday-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "friday-backend"
        "backend" = "friday-backend"
        "meeting" = "friday-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "friday-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, friday-backend"
        ...
        $logArgs += @("whisper-server", "friday-backend")
    }
