...
from gemini_processor import FridayExtractor
import time
...
class FridayExtractRequest(BaseModel):
    """Request model for Friday's Gemini-based extraction."""
    transcript: str
...
@app.post("/friday-extract")
async def friday_extract(request: FridayExtractRequest):
    """
    Friday endpoint: send a raw transcript to Gemini and get back
    structured JSON (summary, action_items, deadlines).
    Result is also persisted to friday_state.json for agent consumption.
    """
    try:
        extractor = FridayExtractor(api_key=request.api_key)
        result = await extractor.extract(request.transcript)
        return JSONResponse(content=result)
    except ValueError as e:
        logger.error("Friday extraction failed: %s", e)
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        logger.error("Friday extraction error: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
