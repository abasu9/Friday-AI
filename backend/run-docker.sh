...
show_log_exit_options() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "whisper-server\|friday-backend"; then
...
find_existing_databases() {
    local found_dbs=()
    local default_db_path="/opt/homebrew/Cellar/friday-backend/0.0.4/backend/meeting_minutes.db"
    ...
    local common_paths=(
        "/opt/homebrew/Cellar/friday-backend/*/backend/meeting_minutes.db"
        "$HOME/.friday/meeting_minutes.db"
        "$HOME/Documents/friday/meeting_minutes.db"
...
start_server() {
    ...
    if docker images --format "{{.Repository}}:{{.Tag}}" | grep -q "friday-backend:"; then
...
show_status() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "friday-backend"; then
        app_running=true
        local app_port=$(docker port friday-backend 5167/tcp 2>/dev/null | cut -d: -f2)
        if [ -n "$app_port" ]; then
            log_info "Meeting App: http://localhost:$app_port"
            # Test connectivity
