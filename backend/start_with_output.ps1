...
Write-Host "====================================="
Write-Host "Friday Backend Startup"
Write-Host "====================================="
...
Write-Host "====================================="
Write-Host "Starting Friday Backend"
Write-Host "====================================="
...
# Check if friday-frontend is installed
$frontendInstalled = $false
$frontendPath = $null

# Check common installation paths for friday-frontend
$possiblePaths = @(
    "$env:LOCALAPPDATA\Programs\friday-frontend\friday-frontend.exe",
    "$env:LOCALAPPDATA\Programs\friday\friday-frontend.exe",
    "$env:ProgramFiles\friday-frontend\friday-frontend.exe",
    "${env:ProgramFiles(x86)}\friday-frontend\friday-frontend.exe",
    "$env:APPDATA\friday-frontend\friday-frontend.exe"
)
...
try {
    ...
    if ($regPath) {
        ...
        if (-not $frontendPath -and $regPath.InstallLocation) {
            ...
            $possibleExeNames = @("friday-frontend.exe", "friday.exe")
...
if ($frontendInstalled) {
    Write-Host "Friday frontend application is installed."
    if ($frontendPath) {
        Write-Host "Location: $frontendPath"
        
        # Ask if user wants to launch the frontend
        $launchFrontend = Read-Host "Do you want to launch the Friday frontend application? (Y/N)"
        if ($launchFrontend -eq 'Y' -or $launchFrontend -eq 'y') {
            Write-Host "Launching Friday frontend..."
            Start-Process -FilePath $frontendPath
            Write-Host "Friday frontend launched successfully."
        }
    }
} else {
    Write-Host "Friday frontend application is not installed."
    Write-Host ""
    $installFrontend = Read-Host "Would you like to download and install the Friday frontend application? (Y/N)"
        ...
        try {
            ...
            if ($setupAsset) {
                ...
                if ($installerProcess.ExitCode -eq 0) {
                    ...
                    # Check if friday is now installed and launch it
