...
Injects CSV-based transcript data into the Friday SQLite database,
...
def get_default_db_path() -> Path:
    ...
    if system == "Darwin":  # macOS
        base_path = Path.home() / "Library" / "Application Support" / "Friday"
    elif system == "Windows":
        appdata = os.environ.get("APPDATA", "")
        if appdata:
            base_path = Path(appdata) / "Friday"
        else:
            base_path = Path.home() / "AppData" / "Roaming" / "Friday"
    else:  # Linux and others
        base_path = Path.home() / ".config" / "Friday"
...
def main():
    parser = argparse.ArgumentParser(
        description="Inject CSV transcript data into Friday database",
...
Example usage:
  ...
  python inject_transcript.py --csv data.csv --db ~/custom/path.sqlite
    ...
    if not db_path.exists():
        ...
        print("Make sure Friday has been run at least once to create the database.", file=sys.stderr)
        sys.exit(1)
    ...
    print("\nThe meeting should now appear in the Friday sidebar.")
